...
df_broccoli2 = prepare_post_pred(fit_broccoli, df_broccoli1, "dallcomp")
df_paprika2 = prepare_post_pred(fit_paprika, df_paprika1, "dallcomp")

df_pred = bind_rows(df_broccoli2, df_paprika2) |>
  factor_vars() |>
  group_by(species, asl, leaf_age, light_treatment) |>
  point_interval(d_all_comp)

# Figure 5
ggplot(d13C_asl, aes(asl, d_all_comp, fill = light_treatment, shape = leaf_age)) +
  ...
  ylab(expression(paste(phantom()^13, "C depletion of adax to abax wax ", 
